...
const INITIAL_STAFF: StaffMember[] = [
  {
    ...
    name: 'Julie Carter',
    role: 'Senior nurse',
    skills: ['Clinical lead', 'First aid'],
...
function buildSchedule(staff: StaffMember[], leaves: LeaveMap): BuildResult {
  ...
  for (const date of WEEK) {
    ...
    if (chosen.length < 3) {
      issues.push(`${date.label} ${date.date}: only ${chosen.length} of 3 required staff can be assigned.`);
    }
    if (!chosen.some((member) => member.skills.includes('Clinical lead'))) {
      issues.push(`${date.label} ${date.date}: no clinical lead is available.`);
    }
    if (!chosen.some((member) => member.skills.includes('First aid'))) {
      issues.push(`${date.label} ${date.date}: no first-aid-qualified staff member is available.`);
    }
  }
...
function findStaff(text: string, staff: StaffMember[]) {
  ...
  return staff.find((member) => {
    ...
    return lower.includes(first) || lower.includes(member.name.toLowerCase());
  });
}
...
function formatDays(days: number[]) {
  if (!days.length) return 'the requested dates';
...
function riskForDay(staff: StaffMember[], leaves: LeaveMap, day: number, weekday: Weekday) {
  const available = staff.filter(
    (member) => member.active && member.availableDays.includes(weekday) && !hasLeave(leaves, member.id, day),
  );
...
export default function RotaReasonClient() {
  ...
  const propose = (nextStaff: StaffMember[], nextLeaves: LeaveMap, label: string, summary: string) => {
    ...
    if (result.issues.length) {
      ...
      addMessage({
        role: 'assistant',
        body: `No — I would not approve that change yet. ${result.issues[0]}`,
        tone: 'warning',
          ...
          <form className={styles.commandForm} onSubmit={submit}>
            ...
            <div className={styles.inputRow}>
              <input
                ...
                placeholder="e.g. Julie wants 17th and 18th off"
                autoComplete="off"
              />
