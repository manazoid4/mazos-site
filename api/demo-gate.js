...
function gateHtml(slug, error = '') {
  const safeSlug = slug.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;');
  const errorHtml = error ? `<p class="error" role="alert">${error}</p>` : '';
  return `<!doctype html>
<html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="robots" content="noindex,nofollow"><title>Private client demo — Maz Works</title>
<style>*{box-sizing:border-box}body{margin:0;min-height:100svh;background:#f5f5ef;color:#111;font-family:Arial,Helvetica,sans-serif;display:grid;place-items:center;padding:24px}.gate{width:min(100%,520px);border-top:4px solid #111;padding:28px 0}.mark{font-weight:900;letter-spacing:-.04em;font-size:28px}.eyebrow{margin-top:54px;font:700 11px/1.3 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.1em;text-transform:uppercase;color:#666}h1{font-size:clamp(42px,9vw,72px);line-height:.92;letter-spacing:-.065em;margin:15px 0 20px}.copy{max-width:390px;color:#565656;line-height:1.55}form{margin-top:36px;display:grid;gap:12px}label{font-size:12px;font-weight:800}input{min-height:54px;border:2px solid #111;background:#fff;padding:0 15px;font:inherit;border-radius:0}button{min-height:54px;border:2px solid #111;background:#111;color:#fff;font-weight:800;padding:0 18px;cursor:pointer}button:hover{background:#cbff00;color:#111}.error{margin:8px 0 0;color:#9d1c1c;font-weight:700;font-size:13px}.small{font-size:11px;color:#777;margin-top:22px}</style></head>
<body><main class="gate"><div class="mark">MAZ WORKS</div><p class="eyebrow">Private client demo</p><h1>Built for the<br>conversation.</h1><p class="copy">Enter the passcode Maz Works sent you to view this private concept.</p>${errorHtml}<form method="post" action="/demos/${safeSlug}"><label for="passcode">Demo passcode</label><input id="passcode" name="passcode" type="password" autocomplete="current-password" required maxlength="72"><button type="submit">Open private demo →</button></form><p class="small">Private concept · Noindex · Access can be revoked</p></main></body></html>`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!result.data?.ok) {
      ...
      return sendHtml(res, gateHtml(slug, 'Your demo session has expired. Enter the passcode again.'), 401);
    }

    // Protected client assets are deliberately served by this function once the
    // per-client content bundle is installed. Until then, never fall through to
    // a public file path that could bypass the session check.
    res.statusCode = 503;
    res.setHeader('content-type', 'text/plain; charset=utf-8');
    res.setHeader('cache-control', 'private, no-store, max-age=0');
    res.setHeader('x-robots-tag', 'noindex, nofollow');
    return res.end('Private demo content bundle is not installed on this deployment yet.');
  } catch (error) {
    console.error('demo-gate failed', error instanceof Error ? error.message : 'unknown error');
    return sendHtml(res, gateHtml(slug, 'The private demo service is temporarily unavailable.'), 503);
  }
}
